...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-ink-950 pt-28 pb-20 text-white sm:pt-36 sm:pb-28">
      {/* Aurora + grid backdrop */}
      <div className="pointer-events-none absolute inset-0 bg-grid opacity-50" />
      <div className="pointer-events-none absolute -left-32 -top-24 size-[34rem] rounded-full bg-brand-600/30 blur-3xl animate-aurora" />
      <div className="pointer-events-none absolute -right-24 top-20 size-[30rem] rounded-full bg-accent-600/20 blur-3xl animate-aurora [animation-delay:-6s]" />
        ...
        <div className="grid items-center gap-14 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="flex flex-col items-start gap-6">
            ...
            <Reveal delay={0.05}>
              <h1 className="text-balance text-4xl font-extrabold leading-[1.05] tracking-tight sm:text-6xl lg:text-[4.2rem]">
                Restoring{" "}
                <span className="text-gradient">faces, jaws</span>
            ...
            <Reveal delay={0.2}>
              <ul className="mt-2 flex flex-wrap gap-x-6 gap-y-3">
                {trustPoints.map((t) => (
                  ...
                  >
                    <t.icon className="size-4.5 text-brand-400" />
                    {t.label}
                  </li>
                ))}
              </ul>
          ...
          <Reveal direction="left" delay={0.15}>
            <div className="relative">
              <div className="absolute -inset-3 rounded-[2rem] bg-gradient-to-br from-brand-500/20 to-accent-500/20 blur-2xl" />
