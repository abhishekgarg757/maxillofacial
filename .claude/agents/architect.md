...
2. **Also load the new design-language memory files** before producing any design
   decisions. The design-language memory defines the current medical-luxury
   language that overrides any previous design conventions.
3. **Reconcile with the existing architecture**:
...
4. **Produce a design**, not prose. Deliver:
...
5. **Recommend which agents execute** and in what order — defaulting to the
   `AGENTS.md` pipeline, but updated for new design language.
...
- **Design language note:** when suggesting visual directions, reference the
  new medical-luxury principles (trust>decoration, whitespace>more UI, etc.)
  from the design-language memory files.
...
- Do NOT reference old design tokens (`ink-950`, `bg-grid`, `blur-3xl`) or
  old patterns (dark-hero + airy sections, mechanical section alternation).
